...
import { getAllDonVi } from '@/services/donvi';
import { IOrganizationRecord } from '@/utils/interfaces';

export default () => {
  const [danhSach, setDanhSach] = useState<IOrganizationRecord[]>([]);
  const [loading, setLoading] = useState(false);
  const [edit, setEdit] = useState(false);
  const [record, setRecord] = useState<IOrganizationRecord | {}>({});
  const [visibleForm, setVisibleForm] = useState(false);
  const [total, setTotal] = useState(0);
  const [page, setPage] = useState(1);
  const [limit, setLimit] = useState(10);

  const get_all_don_vi = async () => {
    try {
      setLoading(true);
      const res = await getAllDonVi();
      if (res.status === 200) {
